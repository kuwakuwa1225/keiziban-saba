...
app = Flask(__name__)
app.secret_key = "your-secret-key"  # セキュアな値にすること（例: os.urandom(24).hex()）

# LINEチャネル情報（自身の設定に合わせて変更）
CHANNEL_ID = "2007643838"
CHANNEL_SECRET = "7bdea19f8f4a816cdd5e19c7bb6dbbbe"
REDIRECT_URI = "https://keiziban-saba.onrender.com/callback"  # ←注意：ここはエンコードしない「通常のURL」

@app.route("/")
def index():
    return '<h1>ようこそ！</h1><a href="/login">LINEログイン</a>'
...
def mypage():
    if "user_name" not in session:
        return redirect(url_for("index"))  # 未ログインならトップへ
    return f"<h2>こんにちは、{session['user_name']}さん！</h2><p>ここはあなた専用のマイページです。</p>"
